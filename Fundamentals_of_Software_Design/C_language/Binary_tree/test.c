...
void test_unsorted_insert() {
    ...
    bst_destroy(tree);
    printf("Test unsorted insert passed\n");
}
...
void test_sorted_order() {
    ...
    int last_value = -1;
    int count_visited = 0;
    void check_sorted(Pointer data, Pointer extra_data) {
        (void)extra_data;
        int value = *(int*)data;
    ...
    bst_foreach(tree, check_sorted, NULL);
    assert(count_visited == count);
    bst_destroy(tree);
    printf("Test sorted order passed\n");
}
...
int main() {
    ...
    test_foreach();
    test_string_keys();
    test_unsorted_insert();
    test_clear();
    test_sorted_order();
    printf("All binary tree tests passed successfully!\n");
    return 0;
}
