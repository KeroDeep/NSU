...
class MDPCapacitanceApplication:
    ...
    def refresh_interface(self):
        ...
        saved_params = {
            ...
            'donor_level': self.donor_level_variable.get(),
            'doping_concentration_log': self.doping_concentration_log_variable.get(),
        ...
        self.donor_level_variable.set(saved_params['donor_level'])
        self.doping_concentration_log_variable.set(saved_params['doping_concentration_log'])
    ...
    def calculate_capacitance(self):
        """Основной расчёт характеристик МДП-структуры в режиме обеднения (упрощённая модель)"""
        try:
            bandgap = self.bandgap_variable.get()
            ...
            # Диапазон напряжений от 0 до -Vmax (отрицательное для n-типа в режиме обеднения)
    ...
    def update_analysis(self, voltages, capacitances, surface_potentials, depletion_widths, electric_fields):
        ...
        analysis_text += f"{language['doping_concentration']} {10**self.doping_concentration_log_variable.get():.2e} см⁻³\n"
