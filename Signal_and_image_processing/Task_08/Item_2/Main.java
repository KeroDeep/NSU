...
public class Main {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            try {
                mainFrame = new JFrame("Objects contours selecting with size analysis");
    ...
    private static void displayResults() {
        for (int i = 0; i < originalColorImages.size(); i++) {
            ...
            String sizeInfo = analyzeObjectSizeSimple(contours, colorOriginal.getWidth(), colorOriginal.getHeight());
            ...
            addSizeInfoToPanel(rowPanel, sizeInfo, colorOriginal.getWidth(), colorOriginal.getHeight());
    ...
    private static String analyzeObjectSizeSimple(List<List<Point>> contours, int imageWidth, int imageHeight) {
        if (contours.isEmpty()) {
        ...
        for (List<Point> contour : contours) {
            if (contour.size() > 10) {
                double area = calculateBoundingBoxArea(contour);
                if (area > maxArea) {
                    maxArea = area;
        ...
        double areaPercentage = (maxArea / imageArea) * 100;
        
        String sizeCategory;
        if (areaPercentage < 10) {
            sizeCategory = "маленький";
        } else if (areaPercentage < 30) {
            sizeCategory = "средний";
        } else {
            sizeCategory = "большой";
        }
        
        return String.format("Area: %.1f%%\nSize: %s\nContours: %d", 
                           areaPercentage, sizeCategory, contours.size());
    }

    private static double calculateBoundingBoxArea(List<Point> contour) {
    ...
    private static void addSizeInfoToPanel(JPanel parent, String sizeInfo, int imageWidth, int imageHeight) {
        ...
        JLabel titleLabel = new JLabel("Size Analysis", JLabel.CENTER);
        ...
        container.add(new JScrollPane(infoText), BorderLayout.CENTER);
